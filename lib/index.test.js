...
describe('#index', () => {
  ...
  describe('#constructor()', () => {
    ...
    it('should run invoke:stepf:invoke promise chain in order', () => {
      ...
      const manifestParseStub = sinon
        .stub(serverlessStepFunctions, 'manifestParse').returns(BbPromise.resolve());
      return serverlessStepFunctions.hooks['invoke:stepf:invoke']()
        .then(() => {
          expect(invokeStub.calledOnce).to.be.equal(true);
          expect(manifestParseStub.calledOnce).to.be.equal(true);
          serverlessStepFunctions.manifestParse.restore();
