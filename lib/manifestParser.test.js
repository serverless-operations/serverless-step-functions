...
describe('#manifestParse', () => {
  ...
  describe('#manifestParse()', () => {
    ...
    beforeEach(() => {
      ...
      getRawConfigStub = sinon.stub(serverlessStepFunctions, 'getRawConfig')
    ...
    afterEach(() => {
      serverlessStepFunctions.getRawConfig.restore();
    ...
    it('should default to dev when stage and provider are not defined', () => {
      ...
      serverlessStepFunctions.manifestParse()
        .then(() => {
    ...
    it('should throw error if servicePath is not given', () => {
      ...
      serverlessStepFunctions.manifestParse()
        .then(() => {
          expect(getRawConfigStub.calledOnce).to.be.equal(false);
    ...
    it('should create corresponding when stepfunctions param are given', () => {
      ...
      serverlessStepFunctions.manifestParse()
        .then(() => {
          expect(getRawConfigStub.calledOnce).to.be.equal(true);
    ...
    it('should default to serverless.yml if serviceFileName (--config option) is not passed', () => {
      ...
      serverlessStepFunctions.manifestParse()
        .then(() => {
          expect(getRawConfigStub.calledWith(`${servicePath}/serverless.yml`)).to.be.equal(true);
        });
    });
    ...
    it('should read serviceFileName if passed as --config option', () => {
      ...
      serverlessStepFunctions.manifestParse()
        .then(() => {
          expect(getRawConfigStub.calledWith(`${servicePath}/${fileName}`)).to.be.equal(true);
        });
    });
  });
