            ...
            if (typeof event.schedule === 'object') {
              ...
              if (InputTransformer) {
                if (method === METHOD_SCHEDULER) {
                  const errorMessage = [
                    'Cannot setup "schedule" event: "inputTransformer" is not supported '
                    + 'with "scheduler" mode',
                  ].join('');
                  throw new this.serverless.classes
                    .Error(errorMessage);
              ...
              if (InputPath && method === METHOD_SCHEDULER) {
                const errorMessage = [
                  'Cannot setup "schedule" event: "inputPath" is not supported '
                  + 'with "scheduler" mode',
                ].join('');
                throw new this.serverless.classes
                  .Error(errorMessage);
              ...
              if (timezone && method !== METHOD_SCHEDULER) {
                const errorMessage = [
                  'Cannot setup "schedule" event: "timezone" is only supported '
                  + 'with "scheduler" mode',
                ].join('');
                throw new this.serverless.classes
                  .Error(errorMessage);
