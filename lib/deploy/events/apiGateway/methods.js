  ...
  getMethodIntegration(stateMachineName, stateMachineObj, http) {
    ...
    if (http && http.cors) {
      ...
      const preflightHeaders = {
        ...
        ...(_.has(http.cors, 'headers') && { 'Access-Control-Allow-Headers': http.cors.headers.join(',') }),
        ...(_.has(http.cors, 'methods') && { 'Access-Control-Allow-Methods': http.cors.methods.join(',') }),
        ...(_.has(http.cors, 'allowCredentials') && { 'Access-Control-Allow-Credentials': http.cors.allowCredentials }),
      };

      // Enable CORS Max Age usage if set
      if (_.has(http.cors, 'maxAge')) {
        if (_.isInteger(http.cors.maxAge) && http.cors.maxAge > 0) {
          preflightHeaders['Access-Control-Max-Age'] = http.cors.maxAge;
        } else {
      ...
      if (_.includes(http.cors.methods, 'ANY')) {
      ...
      integrationResponse.IntegrationResponses.forEach((val, i) => {
        Object.keys(preflightHeaders).forEach((key) => {
          integrationResponse.IntegrationResponses[i]
            .ResponseParameters[`method.response.header.${key}`] = `'${preflightHeaders[key]}'`;
        });
      });
    }
  ...
  getMethodResponses(http) {
    ...
    if (http && http.cors) {
      const preflightHeaders = {
        ...
        ...(_.has(http.cors, 'headers') && { 'Access-Control-Allow-Headers': true }),
        ...(_.has(http.cors, 'methods') && { 'Access-Control-Allow-Methods': true }),
        ...(_.has(http.cors, 'allowCredentials') && { 'Access-Control-Allow-Credentials': true }),
      };
      ...
      if (_.includes(http.cors.methods, 'ANY')) {
        preflightHeaders['Access-Control-Allow-Methods'] = true;
      }
