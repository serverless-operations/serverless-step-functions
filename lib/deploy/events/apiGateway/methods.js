...
module.exports = {
  ...
  getMethodIntegration(stateMachineName, stateMachineObj, http) {
    ...
    if (_.has(http, 'responses')) {
      const responses = _.get(http, 'responses', {});
      ...
      _.forOwn(responses, (value, key) => {
        integrationResponse.IntegrationResponses.push({
          StatusCode: _.toInteger(key),
          SelectionPattern: _.toInteger(key),
          ResponseParameters: _.get(value, 'responseParameters', {}),
          ResponseTemplates: _.get(value, 'responseTemplates', {}),
        });
      });
    } else {
