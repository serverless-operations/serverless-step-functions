...
module.exports = {
  compileHttpLambdaPermissions() {
    const resources = _.get(this.serverless, 'service.provider.compiledCloudFormationTemplate.Resources', {});
    const authorizers = _.values(resources).filter((r) => r.Type === 'AWS::ApiGateway::Authorizer');
    const customAuthorizers = authorizers.filter(
      (r) => r.Properties.Type === 'CUSTOM' || r.Properties.Type === 'TOKEN',
    );
