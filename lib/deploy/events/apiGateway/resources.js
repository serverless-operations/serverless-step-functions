...
module.exports = {
  ...
  getResourcePaths() {
    ...
    if (_.every(
      this.pluginhttpValidated.events,
      (event) => _.some(predefinedResourceNodes, (node) => node.path === event.http.path),
    )) {
    ...
    const sortedResourceNodes = _.sortBy(
      predefinedResourceNodes,
      (node) => node.level,
    );
  ...
  getResourceId(path) {
    ...
    if (!this.apiGatewayResources[path].resourceId
      ...
      this.apiGatewayResources[path]
        .resourceId = { Ref: this.apiGatewayResources[path].resourceLogicalId };
    }
