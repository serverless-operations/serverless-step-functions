...
describe('#compileStateMachines', () => {
  ...
  beforeEach(() => {
    ...
    provider.getAccountId = () => Promise.resolve(accountId);
  ...
  it('should create corresponding resources when definition and name property is given', () => {
    ...
    return serverlessStepFunctions.compileStateMachines().then(() => {
  ...
  it('should throw error when definition property is not given', () => {
    ...
    expect(serverlessStepFunctions.compileStateMachines()).to.eventually.be.rejectedWith(Error);
  });
  ...
  it('should replace only the function name in resource with the arn', () => {
    const expectedArn = 'arn:aws:lambda:us-east-1:12345678:step-functions-dev-us-east-1-test-task';
