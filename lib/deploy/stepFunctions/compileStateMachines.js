...
module.exports = {
  replaceNamesWithArns(stateMachineName, accountId, node) {
    if (node instanceof Array) {
      for (let i = 0; i < node.length; i++) {
        this.replaceNamesWithArns(stateMachineName, accountId, node[i]);
      }
    ...
    for (let i = 0; i < keys.length; i++) {
      if (node[keys[i]] instanceof Array || node[keys[i]] instanceof Object) {
        this.replaceNamesWithArns(stateMachineName, accountId, node[keys[i]]);
      }
    }
      ...
      return this.provider.getAccountId().then((accountId) => {
        this.getAllStateMachines().forEach((stateMachineName) => {
          ...
          if (stateMachineObj.definition) {
            this.replaceNamesWithArns(stateMachineName, accountId, stateMachineObj.definition);
