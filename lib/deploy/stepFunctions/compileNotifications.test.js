...
describe('#compileNotifications', () => {
  ...
  const validateHasSnsPolicy = (resources, topicArn) => {
    const snsPolicies = _.values(resources).filter(resource =>
      resource.Type === 'AWS::SNS::TopicPolicy');
    const matchedPolicy = snsPolicies.find(policy =>
      _.isEqual(policy.Properties.Topics[0], topicArn));
    expect(matchedPolicy).to.not.equal(
      undefined,
  ...
  const validateHasSqsPolicy = (resources, queueUrl) => {
    const sqsPolicies = _.values(resources).filter(resource =>
      resource.Type === 'AWS::SQS::QueuePolicy');
    const matchedPolicy = sqsPolicies.find(policy =>
      _.isEqual(policy.Properties.Queues[0], queueUrl));
    expect(matchedPolicy).to.not.equal(
      undefined,
  ...
  const validateHasLambdaPermission = (resources, lambdaArn) => {
    const lambdaPolicies = _.values(resources).filter(resource =>
      resource.Type === 'AWS::Lambda::Permission');
    const matchedPolicy = lambdaPolicies.find(policy =>
      _.isEqual(policy.Properties.FunctionName, lambdaArn));
    expect(matchedPolicy).to.not.equal(
      undefined,
